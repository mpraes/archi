...
PT | EN
May 30, 2026 · 💬 Join the Discussion
...
Installation surface. The new user has to be able to install and try the tool with the least friction possible. One command, ideally.
Tests and automated CI. So contributions are easier and there’s common ground on what’s minimally acceptable to land in the project.
Documentation. The short, easy-to-read version (the README) and a more detailed set in docs/, so new contributors understand the architectural decisions.
